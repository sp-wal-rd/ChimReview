...
/** @defgroup SSI0_Group1 Initialization and Configuration functions
                   ...
                   SPI_TXE_INT_TypeDef SPI_TXE_INT)
{
  OTCON = (OTCON & 0XCF) | 0X10;
	SPI_TXE_INT = SPI_TXE_DISINT;//SPI_TXE_INT该功能在SSI0芯片上无效
...
/** @defgroup SSI0_Group2 Base functions
 *  @brief   SSI0 functions
 *
@verbatim
 ===============================================================================
                     ##### Base functions #####
...
StatusTypeDef SSI0_SPI_Receive(SSI_HandleInfoDef* SSI_HANDLE, uint8_t* pData, uint8_t Size, uint32_t Timeout)
{
  ...
  if(SSI_HANDLE->RxState == SSI_STATE_READY)
  {
    ...
    if(((IE & 0x10) != 0) || (Size == 0U))
    {
      return SSI_STATE_ERROR;
    }

    /* SPI作为主机时，接收数据必定跟随发送数据 */
    if((SS0CON0 & SPI_MODE_MASTER) != 0)
    {
      return SSI0_SPI_TransmitReceive(SSI_HANDLE, pData, pData, Size, Timeout);			//跳转到阻塞模式数据收发函数
    }

    SSI_HANDLE->RxState = SSI_STATE_BUSY;			//发送进程忙碌中
    SSI0_ClearFlag(SPI_FLAG_SPIF);			 //接收前清除接收中断标志
    ...
    {
      /* 等待SPI中断标志位置起 */
      if(SSI0_GetFlagStatus(SPI_FLAG_SPIF))
      {
          *(SSI_HANDLE->pRxBuffPtr.Size_u8 + SSI_HANDLE->RxXferCount) = SSI0_SPI_ReceiveData();			//读取16位数据,并且地址自增
        SSI0_ClearFlag(SPI_FLAG_SPIF);				//清除标志位
...
StatusTypeDef SSI0_SPI_Receive_IT(SSI_HandleInfoDef* SSI_HANDLE, uint8_t* pData, uint8_t Size)
{
  ...
  if(SSI_HANDLE->RxState == SSI_STATE_READY)
  {
    ...
    if((SS0CON0 & SPI_MODE_MASTER) != 0)
    {
      return SSI0_SPI_TransmitReceive_IT(SSI_HANDLE, pData, pData, Size);			//跳转到中断模式数据收发函数
    }

    SSI_HANDLE->RxState = SSI_STATE_BUSY;			//状态更新为接收忙碌中
    SSI0_ClearFlag(SPI_FLAG_SPIF);			//清除中断标志位
    SSI_HANDLE->pRxBuffPtr.Size_u8 = pData;			//指向待发送数据的地址
    SSI_HANDLE->RxXferSize = Size;			//更新待发送的数据量
    SSI_HANDLE->RxXferCount = 0;			//发送计数器清零
...
StatusTypeDef SSI0_SPI_Transmit(SSI_HandleInfoDef* SSI_HANDLE, uint8_t* pData, uint8_t Size, uint32_t Timeout)
{
  ...
  if(SSI_HANDLE->TxState == SSI_STATE_READY)
  {
    ...
    SSI0_ClearFlag(SPI_FLAG_SPIF);			 //发送前清除标志位
    ...
    while(SSI_HANDLE->TxXferCount < SSI_HANDLE->TxXferSize)//判断是否接收所有数据
    {
        ...
        SSI0_SPI_SendData(*(SSI_HANDLE->pTxBuffPtr.Size_u8 + SSI_HANDLE->TxXferCount));			//发送8位数据并且数据地址增加
      while(!SSI0_GetFlagStatus(SPI_FLAG_SPIF))			//等待发送完成
      {
        /* 超时计数器不断累加，直到超出设定的超时时间 */
        if(TimeoutCnt++ > Timeout)
        {
      ...
      SSI0_ClearFlag(SPI_FLAG_SPIF);				//发送前先清除标志位
...
StatusTypeDef SSI0_SPI_Transmit_IT(SSI_HandleInfoDef* SSI_HANDLE, uint8_t* pData, uint8_t Size)
{
  ...
  if(SSI_HANDLE->TxState == SSI_STATE_READY)
  {
    ...
    /* 发送第1byte数据 */
      SSI0_SPI_SendData(*(SSI_HANDLE->pTxBuffPtr.Size_u8));
...
StatusTypeDef SSI0_SPI_TransmitReceive(SSI_HandleInfoDef* SSI_HANDLE, uint8_t* pTxData, uint8_t* pRxData, uint8_t Size, uint32_t Timeout)
{
  ...
  if((SSI_HANDLE->TxState == SSI_STATE_READY) && (SSI_HANDLE->RxState == SSI_STATE_READY))
  {
    ...
    while(SSI_HANDLE->TxXferCount < SSI_HANDLE->TxXferSize);
    {
      ...
      SSI0_SPI_SendData(*(SSI_HANDLE->pTxBuffPtr.Size_u8 + SSI_HANDLE->TxXferCount));			//发送16位数据并且地址增加
      while(!SSI0_GetFlagStatus(SPI_FLAG_SPIF))			//等待发送完成
      {
        /* 等待时间是否发生了超时 */
        if(TimeoutCnt++ > Timeout)
      ...
      SSI0_ClearFlag(SPI_FLAG_SPIF);				//发送前先清除标志位
      *(SSI_HANDLE->pTxBuffPtr.Size_u8 + SSI_HANDLE->TxXferCount) = SSI0_SPI_ReceiveData();
    ...
    if(SSI_HANDLE->TxXferCount == SSI_HANDLE->TxXferSize)
    {
      SSI_HANDLE->TxState = SSI_HANDLE->RxState = SSI_STATE_READY;	//发送与接收完成
      return Status_OK;
    }
    else
    {
...
StatusTypeDef SSI0_SPI_TransmitReceive_IT(SSI_HandleInfoDef* SSI_HANDLE, uint8_t* pTxData, uint8_t* pRxData, uint8_t Size)
{
  ...
  if((SSI_HANDLE->TxState == SSI_STATE_READY) && (SSI_HANDLE->RxState == SSI_STATE_READY))
  {
    ...
    /* 发送第1byte数据 */
    /* SPI处于8位通信模式 */
    SSI0_SPI_SendData(*(SSI_HANDLE->pTxBuffPtr.Size_u8));			//发送8位数据
 ...
 *              - SET:操作成功
 */
FlagStatus SSI0_GetTWIStatus(SSI_TWIState_TypeDef SSI0_TWIState)
{
  if((SS0CON0&0x07) == SSI0_TWIState)
    return SET;
  else
    return RESET;
}
...
{
  uint32_t TimeoutCnt = 0;
  while(!(SSI0_GetFlagStatus(TWI_FLAG_TWIF)))			//等待启动信号发送完毕
  {
    TimeoutCnt++;
    if(TimeoutCnt > Timeout)
    {
  ...
  SSI0_ClearFlag(TWI_FLAG_TWIF);			//清除标志位
  return Status_OK;
}
...
StatusTypeDef SSI0_TWI_Slave_Transmit(SSI_HandleInfoDef* SSI_HANDLE, uint8_t* pData, uint8_t Size, uint32_t Timeout)
{
  ...
  if(SSI_HANDLE->TxState == SSI_STATE_READY)
  {
    ...
    {
      SSI_HANDLE->TxState = SSI_STATE_BUSY;			//发送进程忙碌中
      SSI0_ClearFlag(TWI_FLAG_TWIF);			 //发送前清除标志位
      SSI_HANDLE->pTxBuffPtr.Size_u8 = pData;			//指向待发送数据的地址
      SSI_HANDLE->TxXferSize = Size;			//更新待发送的数据量
      SSI_HANDLE->TxXferCount = 0;			//发送计数器清零

      
      SSI0_TWI_AcknowledgeConfig(ENABLE);         //开启AA使能位
      if(SSI0_TWI_Wait_TWIF(SSI_HANDLE, Timeout) == Status_TIMEOUT)            //等待地址匹配
      {
        SSI0_TWI_AcknowledgeConfig(DISABLE);            //关闭AA使能
        return Status_TIMEOUT;
      }

      do
      {
        if(SSI0_GetTWIStatus(TWI_SlaveSendData) == SET)           //检测状态机状态
        {

          SS0CON2 = *(SSI_HANDLE->pTxBuffPtr.Size_u8 + SSI_HANDLE->TxXferCount);             //TWI发送数据

          if(SSI0_TWI_Wait_TWIF(SSI_HANDLE, Timeout) == Status_TIMEOUT)             //等待启动信号发送完毕
          {
            SSI0_TWI_AcknowledgeConfig(DISABLE);            //关闭AA使能
            return Status_TIMEOUT;
          }
        ...
        else if(SSI0_GetTWIStatus(TWI_SlaveDisableACK) == SET)
        {
      ...
      if(SSI0_GetTWIStatus(TWI_SlaveDisableACK) == RESET)           //检测状态机状态
      {
        SSI_HANDLE->TxState = SSI_STATE_ERROR;
        return Status_ERROR;
      }
...
StatusTypeDef SSI0_TWI_Slave_Transmit_IT(SSI_HandleInfoDef* SSI_HANDLE, uint8_t* pData, uint8_t Size)
{
  ...
  if(SSI_HANDLE->TxState == SSI_STATE_READY)
  {
    ...
    else
    {
      ...
      SSI0_TWI_AcknowledgeConfig(DISABLE);         //开启AA
      SSI0_ClearFlag(TWI_FLAG_TWIF);			 //发送前清除标志位
      ...
      SSI0_TWI_AcknowledgeConfig(ENABLE);         //开启AA
...
StatusTypeDef SSI0_TWI_Slave_Transmit_IRQHandler(SSI_HandleInfoDef* SSI_HANDLE)
{
  if(SSI_HANDLE->TxState == SSI_STATE_BUSY)
  {
    ...
    if(SSI0_GetTWIStatus(TWI_SlaveSendData) == SET)
    {
      if(SSI_HANDLE->TxXferCount < SSI_HANDLE->TxXferSize)
      {
        SSI_HANDLE->TxXferCount++;			//第一次中断是接受到地址了
        SS0CON2 = *(SSI_HANDLE->pTxBuffPtr.Size_u8 + SSI_HANDLE->TxXferCount);
        if(SSI_HANDLE->TxXferCount == SSI_HANDLE->TxXferSize - 1)
        {
          SSI0_TWI_AcknowledgeConfig(DISABLE);         //关闭AA使能位
        }
        return Status_BUSY;
      }
      else
    ...
    2.接收到主机回来的UACK */
    else if((SSI0_GetTWIStatus(TWI_SlaveDisableACK) == SET) || (SSI0_GetTWIStatus(TWI_SlaveReceivedaUACK) == SET))
    {
      SSI0_TWI_AcknowledgeConfig(DISABLE);         //关闭AA使能位
		 SSI_HANDLE->TxXferCount++;
...
StatusTypeDef SSI0_TWI_Slave_Receive(SSI_HandleInfoDef* SSI_HANDLE, uint8_t* pData, uint8_t Size, uint32_t Timeout)
{
  if(SSI_HANDLE->RxState == SSI_STATE_READY)
  {
    ...
    SSI0_ClearFlag(TWI_FLAG_TWIF);			 //接收前清除标志位
    SSI0_TWI_AcknowledgeConfig(ENABLE);         //开启AA使能位
    ...
    /* 接收地址工程 */
    if(SSI0_TWI_Wait_TWIF(SSI_HANDLE, Timeout) == Status_TIMEOUT)            //等待信号发送完毕
    {
    ...
    if(SSI0_GetTWIStatus(TWI_SlaveReceivedaData) == RESET)           //检测状态机状态
    {
    ...
    do
    {
      ...
      if(SSI0_GetTWIStatus(TWI_SlaveReceivedaData) == SET)           //检测状态机状态
      {
        *(SSI_HANDLE->pRxBuffPtr.Size_u8 + SSI_HANDLE->RxXferCount) = SS0CON2;             //TWI接收数据
        SSI_HANDLE->RxXferCount++;
        if(SSI_HANDLE->RxXferCount == SSI_HANDLE->RxXferSize - 1)
          SSI0_TWI_AcknowledgeConfig(DISABLE);         //关闭AA使能位,回主机UAC
      }
      else if(SSI0_GetTWIStatus(TWI_SlaveIdle) == SET)
      {
        if(SSI_HANDLE->RxXferCount == SSI_HANDLE->RxXferSize - 1)
        {
          *(SSI_HANDLE->pRxBuffPtr.Size_u8 + SSI_HANDLE->RxXferCount) = SS0CON2;
          SSI_HANDLE->RxXferCount++;
        }
        else
        {
...
StatusTypeDef SSI0_TWI_Slave_Receive_IRQHandler(SSI_HandleInfoDef* SSI_HANDLE)
{
  if(SSI_HANDLE->RxState == SSI_STATE_BUSY)
  {
    if(SSI0_GetTWIStatus(TWI_SlaveReceivedaData) == SET)           //检测状态机状态
    {
      if(SSI_HANDLE->RxXferCount < SSI_HANDLE->RxXferSize)
      {
        *(SSI_HANDLE->pRxBuffPtr.Size_u8 + SSI_HANDLE->RxXferCount) = SS0CON2;             //TWI发送数据
        SSI_HANDLE->RxXferCount++;
        if(SSI_HANDLE->RxXferCount == SSI_HANDLE->RxXferSize - 1)
        {
          SSI0_TWI_AcknowledgeConfig(DISABLE);         //关闭AA使能位,回主机UACK
        }
      }
      else
      {
        SSI0_TWI_AcknowledgeConfig(DISABLE);
        return Status_ERROR;
      }
      return Status_BUSY;
    }
...
StatusTypeDef SSI0_UART_Transmit(SSI_HandleInfoDef* SSI_HANDLE, uint8_t* pData, uint8_t Size, uint32_t Timeout)
{
  ...
  if(SSI_HANDLE->TxState == SSI_STATE_READY)
  {
    ...
    SSI_HANDLE->TxState = SSI_STATE_BUSY;			//发送进程忙碌中
    SSI0_ClearFlag(UART_FLAG_TI); //发送前清除发送中断标志，写1/写0清零
    SSI_HANDLE->pTxBuffPtr.Size_u8 = pData;			//指向待发送数据的地址
    SSI_HANDLE->TxXferSize = Size;			//更新待发送的数据量
    SSI_HANDLE->TxXferCount = 0;			//发送计数器清零
    ...
    while(SSI_HANDLE->TxXferCount < SSI_HANDLE->TxXferSize)//判断是否接收所有数据
    {
      if((SS0CON0 & 0x80) == UART_Mode_11B)
      {
        /* 发送9位数据 */
        SSI0_UART_SendData9(*(SSI_HANDLE->pTxBuffPtr.Size_u16 + SSI_HANDLE->TxXferCount));
      }
      else
      {
        /* 发送8位数据 */
        SSI0_UART_SendData8(*(SSI_HANDLE->pTxBuffPtr.Size_u8 + SSI_HANDLE->TxXferCount));
      }
      ...
      SSI0_ClearFlag(UART_FLAG_TI); //发送中断标志清除，写1/写0清零
      SSI_HANDLE->TxXferCount ++;	//发送数据量计数
    }
...
StatusTypeDef SSI0_UART_Receive(SSI_HandleInfoDef* SSI_HANDLE, uint8_t* pData, uint8_t Size, uint32_t Timeout)
{
  ...
  {
    /* 检查接收数据量大小和是否开启接收使能 */
    if((IE & 0x10 != 0) || (Size == 0U) || ((SS0CON0 & 0x10) != UART_RX_ENABLE))
    {
      return  Status_ERROR;
    }

    SSI_HANDLE->RxState = SSI_STATE_BUSY;			//接收进程忙碌中
    SSI0_ClearFlag(UART_FLAG_RI);			 //接收前清除接收中断标志
    SSI_HANDLE->pRxBuffPtr.Size_u8 = pData;			//指向待接收数据的地址
    SSI_HANDLE->RxXferSize = Size;			//更新待接收的数据量
    SSI_HANDLE->RxXferCount = 0;			//接收计数器清零
    ...
    {
      if(SSI0_GetFlagStatus(UART_FLAG_RI)) //判断接收标志位
      {
        SSI0_ClearFlag(UART_FLAG_RI);			//接收标志位清零

        /* 待接收数据量未为0，继续发送 */
        if((SS0CON0 & 0x80) == UART_Mode_11B)
        {
          /* 接收9位数据 */
          *(SSI_HANDLE->pRxBuffPtr.Size_u16 + SSI_HANDLE->RxXferCount) = SSI0_UART_ReceiveData9();
        }
        else
        {
          /* 接收8位数据 */
          *(SSI_HANDLE->pRxBuffPtr.Size_u8 + SSI_HANDLE->RxXferCount) = SSI0_UART_ReceiveData8();
        }
...
StatusTypeDef SSI0_UART_Receive_IT(SSI_HandleInfoDef* SSI_HANDLE, uint8_t* pData, uint8_t Size)
{
  ...
  if(SSI_HANDLE->RxState == SSI_STATE_READY)
  {
    /* 检查是否开启SSI0_UART中断或发送数据量大小为0或是否开启接收使能，返回错误 */
    if(((IE & 0x10) == 0) || (Size == 0U) || ((SS0CON0 & 0x10) != UART_RX_ENABLE))
    {
      return Status_ERROR;
    }
